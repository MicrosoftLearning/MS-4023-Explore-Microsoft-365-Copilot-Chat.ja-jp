...
1. 次に、この情報から Word 文書を作成するように Copilot に依頼し、次のプロンプトを入力します。

   +++この計画の Word 文書を簡単に作成します。+++

1. 生成された **Word** ドキュメントを開きます。
...
## タスク 6: Copilot ページ を使用した共同作業

Copilot ページ を使用して、タスク 5 のメールの下書きを共同作業用ワークスペースに変換します。 これにより、複数のチーム メンバーがリアルタイムでコンテンツを調整する方法をシミュレートできます。

1. **Task 5** の Copilot の最後の応答の下に表示される **[Pages で編集]** (または任意の応答) を選択し、その情報を Copilot ページ にコピーします。
